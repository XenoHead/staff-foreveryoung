...
// .wrangler/tmp/bundle-jHidpR/checked-fetch.js
    ...
    for (const k of ["new", "hot", "rare"]) {
      ...
      if (val) {
        ...
        if (refs.length) {
          ...
          const foundRefs = /* @__PURE__ */ new Set();
...
async function handleFeaturedPost(request, env) {
  ...
  for (const k of ["new", "hot", "rare"]) {
    ...
    if (body[k] !== void 0) {
      await db.prepare(`INSERT INTO Settings (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value=excluded.value`).bind(valKey, body[k]).run();
    }
  }
  if (body.new !== void 0) {
    await db.prepare(`INSERT INTO Settings (key, value) VALUES ('featured_items', ?) ON CONFLICT(key) DO UPDATE SET value=excluded.value`).bind(body.new).run();
  }
...
// .wrangler/tmp/bundle-jHidpR/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-jHidpR/middleware-loader.entry.ts
